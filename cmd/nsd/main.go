...
func main() {
	cobra.EnableCommandSorting = false //设置命令排序 true为打开排序
	cdc := app.MakeCodec()
...
func newApp(loger log.Logger, db dbm.DB, traceStore io.Writer) abci.Application {
	return app.NewNameServiceApp(loger, db)
}
func appExporter() server.AppExporter {
	return func(logger log.Logger, db dbm.DB, writer io.Writer, i int64, b bool, strings []string) (json.RawMessage, []tmtypes.GenesisValidator, error) {
		dapp := app.NewNameServiceApp(logger, db)
		return dapp.ExportAppStateAndValidators()
	}
}

// 根据创世文件生成创世状态。初始化tendermint和application的所有文件
...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			genContents, err := os.ReadFile(genFile)
			if err != nil {
				return fmt.Errorf("failed to read file--%s ", genFile)
			}
...
// 返回一个简单的GenTx命令，该命令从一开始就使节点成为验证器
func SimpleAppGenTx(cdc *codec.Codec, pk crypto.PubKey) (appGenTx, cliPrint json.RawMessage, validator tmtypes.GenesisValidator,
	err error) {
	...
	validator = tmtypes.GenesisValidator{PubKey: pk, Power: 10}
